...
var seasonPTSDots = d3
  ...
  .append('svg')
  .attr('width', season_dots_width + season_dots_margin.left + season_dots_margin.right)
  .attr('height', season_dots_height + season_dots_margin.top + season_dots_margin.bottom)
  .append('g')
  .attr('transform', 'translate(' + season_dots_margin.left + ',' + season_dots_margin.top + ')')
...
d3.csv('./files/lakers_game_logs.csv', data => {
  ...
  var seasonPTSDots_x = d3.scaleLinear()
                          .domain([0,season_dots_width])
                          .range([0, season_dots_width])
  var seasonPTSDots_y = d3.scaleLinear()
                          .domain([d3.min(lakersAllGames, d => d.PTS) * 0.95, d3.max(lakersAllGames, d => d.PTS)])
                          .range([season_dots_height, 50])

  // add title and legends
  seasonPTSDots.append('text')
    .attr('x', 0)
    .attr('y', 30)
    .text('Lakers PPG')
  
  seasonPTSDots.append('circle')
    .attr('cx', 100)
    .attr('cy', 25)
    .attr('r', 4)
  ...
  seasonPTSDots.append("g")
  .attr('transform', 'translate(' + 0 + ',' + season_dots_height + ')')
  .call(d3.axisBottom(seasonPTSDots_x).tickSize(0).tickValues([]))
